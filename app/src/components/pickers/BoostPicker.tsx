    ...
    <PickerShell open={open} onClose={onClose} title="Stat boosts">
      <div className="flex flex-col gap-2">
          ...
          return (
            <div key={stat} className="flex items-center gap-2">
              ...
              <button
                ...
                className="w-7 h-7 rounded bg-surface border border-surface-hi text-sm disabled:opacity-30"
              >+</button>
              <div className={`w-9 text-right tabular-nums text-sm ${
                v > 0 ? 'text-ok' : v < 0 ? 'text-danger' : 'opacity-50'
              }`}>
                {v > 0 ? `+${v}` : v}
              </div>
            </div>
        ...
        <div className="flex gap-2 mt-2">
          <button
            onClick={() => setDraft({})}
            className="flex-1 py-2 rounded-lg bg-surface border border-surface-hi text-sm"
          >Reset</button>
          <button
            onClick={commit}
            className="flex-1 py-2 rounded-lg bg-accent-gradient text-white text-sm font-bold"
          >Apply</button>
        </div>
      </div>
    </PickerShell>
