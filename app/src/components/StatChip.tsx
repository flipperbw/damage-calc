interface Props {
  icon?: string;
  label: string;
  tone?: 'default' | 'warn' | 'boost' | 'drop';
  editable?: boolean;
  onClick?: () => void;
}

export function StatChip({ icon, label, tone = 'default', editable, onClick }: Props) {
  // `boost` = green (positive stat stages, +N). `drop` = red (negative
  // stages, -N) so the sign is readable at a glance, not just from the
  // small "-" prefix. `warn` = yellow for non-stat statuses (Burned, etc.).
  const toneClass =
    tone === 'warn'
