      ...
      {slots.map((mon, i) => {
        ...
        if (!mon) {
          return (
            <div
              key={i}
              className="flex-1 aspect-square bg-surface border border-surface-hi rounded-xl flex items-center justify-center opacity-30 text-xs"
            >
              ＋
            </div>
          );
        ...
        return (
          <button
            ...
            className={`flex-1 aspect-square rounded-xl flex items-center justify-center relative ${
              active
